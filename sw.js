const CACHE='tri-ceny-finance-launcher-v16';
const STATIC=['./icons/apple-touch-icon.png?v=16','./icons/icon-192.png?v=16','./icons/icon-512.png?v=16'];
...
self.addEventListener('push',event=>{
  ...
  const options={
    ...
    icon:'./icons/icon-192.png?v=16',badge:'./icons/icon-192.png?v=16',
...
self.addEventListener('notificationclick',event=>{
  ...
  event.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(list=>{
    for(const c of list){if('focus' in c){try{c.navigate('./?source=pwa&v=14');}catch(e){}return c.focus();}}
    return clients.openWindow?clients.openWindow('./?source=pwa&v=14'):undefined;
  }));
});
